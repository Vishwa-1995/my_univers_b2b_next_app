...
const ViewDigitalProducts = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${baseUrl}/offer/fetch_degital`, {
        method: "GET",
      });
            ...
            <table className="w-full h-auto border-collapse mt-10 mb-20 text-sm border border-gray-300 px-20 rounded-[20px] overflow-hidden">
              ...
              <thead className="">
                <tr className="bg-gradient-to-r from-[#02b3bd] to-[#00868e] text-white h-14 text-md">
                  ...
                  <th className="p-2 border border-[#ddd]">Digital Offer Name</th>
              ...
              <tbody>
                { DigitalList.map((offer: any, index) => (
...
export default ViewDigitalProducts