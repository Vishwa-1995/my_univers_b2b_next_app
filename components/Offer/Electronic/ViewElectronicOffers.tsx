...
const ViewElectronicProducts = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${baseUrl}/offer/fetch_electronic`, {
        method: "GET",
      });
      const data = await response.json();
      if (data.data.length > 0) {
        setElectronicList(data.data);
      }
    } catch (error) {
              ...
              <tbody>
                {ElectronicList.map((offer: any, index) => (
                  <tr className="bg-[#f2f2f2] h-12 text-md" key={index}>
                    ...
                    <td className="p-2 border border-[#ddd]">{offer.seller}</td>
                    <td className="p-2 border border-[#ddd]">
                      {offer.department}
                    </td>
...
export default ViewElectronicProducts;
