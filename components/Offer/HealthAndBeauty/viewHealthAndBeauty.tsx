...
const ViewHealthAndBeauty = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${baseUrl}/offer/fetch`, {
        method: "GET",
      });
              ...
              <tbody>
                {OfferList.map((offer: any, index) => (
                  <tr className="bg-[#f2f2f2] h-12 text-md" key={index}>
                    ...
                    <td className="p-2 border border-[#ddd]">{offer.offer_subtype}</td>
                    <td className="p-2 border border-[#ddd]">
...
export default ViewHealthAndBeauty